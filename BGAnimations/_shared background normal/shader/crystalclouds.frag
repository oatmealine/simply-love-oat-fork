...
// Permutations
  ...
  vec4 p = permute( permute( permute( 
             i.z + vec4(-1.0, i1.z, i2.z, 1.0 ))
           + i.y + vec4(-1.0, i1.y, i2.y, 1.0 )) 
           + i.x + vec4(-1.0, i1.x, i2.x, 1.0 ));
...
float gradient(vec2 uv) {
 	return (1.0 + uv.y * uv.y * cloudHeight);  // LITERALLY SEX
}

void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	vec2 uv = (((fragCoord.xy/imageSize.xy)-.5)/imageSize.y)/16; // stupid hack, pls fix
    vec3 p = vec3(uv, ptime*speed);
	...
	fragColor = vec4(q*2/1.25,q*2/1.25,q*2, 1.0);
}
