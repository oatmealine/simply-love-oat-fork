...
#define windowColorA (color2)
#define windowColorB (color3)
...
vec3 window(float z, vec2 pos, vec2 id) {
    ...
    vec3 colA = mix(windowColorA, windowColorB, hash3(id));
    vec3 colB = mix(windowColorA, windowColorB, hash3(id + 0.1));
    vec3 col = mix(colA, colB, hash1(id, level));
    col *= 0.3 + 0.7 * smoothstep(0.1, 0.5, noise(20.0 * pos + 100.0 * hash1(level)));
    col *= smoothstep(windowProb - 0.2, windowProb + 0.2, hash1(id, level + 0.1));
    return col * (0.5 - 0.5 * cos(tau * depth));
}

vec3 addLight(vec3 eye, vec3 ray, float res, float ptime, float height) {
    vec2 q = eye.xy + ((height - eye.z) / ray.z) * ray.xy;

    float row = floor(q.x + 0.5);
    ptime += hash1(row);
    float col = floor(0.125 * q.y - ptime);

    float pos = 0.4 + 0.4 * cos(ptime + tau * hash1(vec2(row, col)));
    vec3 lightPos = vec3(row, 8.0 * (col + ptime + pos), height);
    ...
    vec3 col2 = mix(lightColorA, lightColorB, hash3(vec2(row, col)));
    return mask * light * fog * col2;
}

vec3 addSign(vec3 col, vec3 pos, float side, vec2 id) {
    ...
    flicker = 1.0 - flicker * step(0.96, hash1(charId, ptime));

    col = mix(col, signColor, flash * flicker * step(outline, 0.01) * clamp(-3.5 + 8.0 * noise(id + 6.0 * charPos), 0.0, 1.0));

    outline = smoothstep(0.0, 0.2, outline) * smoothstep(0.5, 0.3, outline);
    return mix(col, outlineColor, flash * outline);
}

void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    vec2 center = -speed * ptime * cameraDir.xy;
	...
	vec3 col = window(p.z - res.y, p.xy, block);

    col = addSign(col, vec3(p.xy - block, p.z - res.y), res.z, block);
    col = mix(vec3(0.0), col, abs(res.z));

    float fog = exp(-fogDensity * max(res.x - fogOffset, 0.0));
    col = mix(color.rgb * 0.4, col, fog);

    float ptime = lightSpeed * ptime;
    col += addLight(eye.xyz, ray.xyz, res.x, ptime, lightHeight - 0.6);
    col += addLight(eye.yxz, ray.yxz, res.x, ptime, lightHeight - 0.4);
    col += addLight(vec3(-eye.xy, eye.z), vec3(-ray.xy, ray.z), res.x, ptime, lightHeight - 0.2);
    col += addLight(vec3(-eye.yx, eye.z), vec3(-ray.yx, ray.z), res.x, ptime, lightHeight);
